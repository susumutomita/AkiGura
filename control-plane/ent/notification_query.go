...
func (_q *NotificationQuery) QueryTeam() *TeamQuery {
...
// QueryWatchCondition chains the current query on the "watch_condition" edge.
func (_q *NotificationQuery) QueryWatchCondition() *WatchConditionQuery {
...
// QuerySlot chains the current query on the "slot" edge.
func (_q *NotificationQuery) QuerySlot() *SlotQuery {
...
func (_q *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withWatchCondition: _q.withWatchCondition.Clone(),
		withSlot:           _q.withSlot.Clone(),
...
func (_q *NotificationQuery) WithTeam(opts ...func(*TeamQuery)) *NotificationQuery {
	query := (&TeamClient{config: _q.config}).Query()
...
// WithWatchCondition tells the query-builder to eager-load the nodes that are connected to
// the "watch_condition" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NotificationQuery) WithWatchCondition(opts ...func(*WatchConditionQuery)) *NotificationQuery {
	query := (&WatchConditionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWatchCondition = query
	return _q
}

// WithSlot tells the query-builder to eager-load the nodes that are connected to
// the "slot" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NotificationQuery) WithSlot(opts ...func(*SlotQuery)) *NotificationQuery {
	query := (&SlotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withTeam; query != nil {
		if err := _q.loadTeam(ctx, query, nodes, nil,
			func(n *Notification, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withWatchCondition; query != nil {
		if err := _q.loadWatchCondition(ctx, query, nodes, nil,
			func(n *Notification, e *WatchCondition) { n.Edges.WatchCondition = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSlot; query != nil {
		if err := _q.loadSlot(ctx, query, nodes, nil,
			func(n *Notification, e *Slot) { n.Edges.Slot = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Team)) error {
...
func (_q *NotificationQuery) loadWatchCondition(ctx context.Context, query *WatchConditionQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *WatchCondition)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
	...
	query.Where(watchcondition.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *NotificationQuery) loadSlot(ctx context.Context, query *SlotQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Slot)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
		fk := nodes[i].SlotID
