...
func (tx *Tx) init() {
	...
	tx.Ground = NewGroundClient(tx.config)
	tx.Municipality = NewMunicipalityClient(tx.config)
	tx.Notification = NewNotificationClient(tx.config)
	tx.PromoCode = NewPromoCodeClient(tx.config)
	tx.PromoCodeUsage = NewPromoCodeUsageClient(tx.config)
	tx.ScrapeJob = NewScrapeJobClient(tx.config)
	...
	tx.WatchCondition = NewWatchConditionClient(tx.config)
}
